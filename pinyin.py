...
def makePinyinTrie():
    '''
    pickle文件不存在时执行该函数，
    尝试打开pinyin.txt文件，如果不存在，使用transFile函数建立
    从pinyin.txt中读取单词，并建立trie树
    最后存储至 pinyin_trie.pkl文件
    ...
    try:
        ...
        print("pinyin.txt文件不存在，去弄一个")

    for word in raw_data:
        trie.add(word[0:-1].lower())
    with open("pinyin_trie.pkl", 'wb') as pkl:  # 以写权限打开二进制文件
...
def handlePwdAndMakeDic(filename, match_module):
    ...
    with open(filename, "r", encoding="utf-8`") as f:
        ...
        while(line):
            ...
            try:
                '''
                line = returnToWord(
                    line[line.rindex(':') + 1:-1].strip())  # -1为了去掉'\n'
                '''
                line = line[line.rindex(':') + 1:-1].strip()  # -1为了去掉'\n'
            except Exception as e:
                errorstr += line

            if not line.isdigit():
                # 如果全是数字，不可能含有拼音，跳过匹配过程
                match = pattern.findall(line)
                if(match):
                    match = "".join(match)

                    # print(match)
                    temp_results = match_module.batchMatch(match)
                    # print(temp_results)
    ...
    with open("pinyin_dic.pkl", 'wb') as pkl:  # 以写权限打开二进制文件
        pickle.dump(dic, pkl)
...
def main():
    '''
    try:
        with open("pinyin_trie.pkl", 'rb') as f:
            trie = Trie(pickle.load(f))
    except Exception as e:
        print("pinyin_trie.pkl文件不存在，正在建立...")
        makePinyinTrie()
        with open("pinyin_trie.pkl", 'rb') as f:
            trie = Trie(pickle.load(f))
    '''
    ...
    '''
    try:
        with open("pinyin_dic.pkl", 'rb') as f:
            dic = pickle.load(f)
    except Exception as e:
        print("pinyin_dic.pkl文件不存在，正在建立...")
        handlePwdAndMakeDic("temp.txt", trie)

        with open("pinyin_dic.pkl", 'rb') as f:
            dic = pickle.load(f)
    '''
    dic = handlePwdAndMakeDic("KZ/163mail_new.txt", trie)

    #长度因素归一化
    for key in dic:
        dic[key]=dic[key]*(math.log2(len(key)))
    ...
    for key in sorted(dic, key=lambda k: dic[k], reverse=1):
        ...
        result_str += '\n'
    with open("pinyin_logone.txt", 'w') as f:
        f.write(result_str)
